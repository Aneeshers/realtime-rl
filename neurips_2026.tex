...
to improve action selection with additional computation. \am{add primer about alphazero in appendix}
...
\begin{figure}[t]
  ...
  \caption{Committed-action execution in real-time Tetris with $K{=}4$.
    ...
    \emph{over} the committed-action delay, not despite it. \am{reflex policy - we sample from it as we think; dark grey; make it clear that is tetris}}
...
\am{add for snake, hex, and sokoban}
...
\begin{figure}[t]
  ...
  \caption{Interpretability: state features conditioned on chosen depth $K$
    ...
    \emph{Top:} In Pac-Man ghost-far states elicit $K{=}4$ (deep planning when safe);
    ghost-close states elicit $K{=}1$ (immediate reaction); rightmost panel shows
    mean nearest-ghost distance by chosen $K$.
    \emph{Bottom:} In Tetris RT, a sparse board elicits $K{=}1$; dense board elicits
...
\begin{figure}[t]
  ...
  \caption{Planning depth frequency over normalized episode time (10 equal bins).
    ...
    Shared legend shown on the right; line styles consistent across all panels. \am{change k 4 dashed}}
  \label{fig:strategy}
\end{figure}
...
For tetris, at a target game speed of 9 FPS, $K{=}4$ planning (128 simulations,
...
\begin{figure}[h]
  ...
  \caption{Validation of the real-time, asynchronous 2-GPU deployment.
    ...
    (placeholder data), validating that the committed-action training protocol perfectly models the deployment delay. \am{change sim-to-real - or justify it}}
