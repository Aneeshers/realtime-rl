...
grows dense.
This adaptive allocation of cognitive effort is so natural, yet it is entirely absent from the standard reinforcement learning (RL) framework \citep{travnik2018reactive, ramstedt2019rtrl}.
...
AlphaZero-style models~\citep{silver2018general} that use Monte Carlo Tree Search (MCTS)
...
that keep the agent moving without full deliberation (Figure \ref{fig:realtime-rl-gating-diagram}); in
...
This design avoids the paradox of planning-about-planning: a single gate forward pass
is orders of magnitude cheaper than one MCTS rollout, so the meta-decision itself
adds negligible real-time cost.
...
the gating policy outperforms every fixed-depth baseline, and the learned strategy concentrates deliberation in
...
\begin{figure}[t]
  ...
  \caption{
    ...
    Given the current state, it selects a planning depth $K$, which determines how
    long MCTS runs and, in committed-action environments, how many reflex actions
    are taken before the planned action is applied.
  }
...
The central idea behind our approach is metareasoning: the agent should reason not
only about which action to take, but also about how much computation that decision
deserves. This view has a long history. Early metareasoning and anytime-planning
work studied how agents should trade computation against acting now
...
deliberation itself as a metalevel MDP
~\citep{lieder2017strategy,griffiths2019doing,callaway2018learning}. Our work
brings that metareasoning perspective into deep RL with modern planners.

\paragraph{Model-based RL has mostly sidestepped it.}
...
just a hardware cost but a meta-reasoning decision whose consequences are paid
directly in game outcomes. Figure~\ref{fig:scaling} makes this tradeoff concrete:
...
We formalize planning-depth selection as a discrete variable-duration control problem.
...
gradient. Making this meta-MCTS AlphaZero stack computationally feasible requires a
number of JAX implementation tricks, which we summarize in Appendix~\ref{app:implementation}.
...
\begin{figure}[t]
  ...
  \caption{
    ...
    committing to a single search depth. Bars show mean $\pm$\,SE over 100
...
the four available simulation-budget options rather than committed-action depth:
...
both fixed-depth policies and hand-designed heuristics (Figure~\ref{fig:strategy},
...
\begin{figure}[t]
  ...
  \caption{The policy plans deeply precisely when the state is dangerous or constrained.
    Across Pac-Man, Tetris, and Snake, larger chosen depths are associated with
    ...
    features conditioned on chosen depth $K$ (mean $\pm$\,1\,SE, 100 episodes).}
...
three environments, and the learned depth distribution also transfers cleanly.
...
exactly one timestep via a 1-step delay RTMDP state augmentation, however, is insufficient for variable-depth MCTS spanning
...
planning depth as a supervision signal for distilling or fine-tuning the underlying
AlphaZero network.
We study a discrete depth set $K \in \Kset$; richer compute vocabularies with more
...
Our measurements confirm that MCTS latency stays well within the frame budget
(H100: mean $122$\,ms, p95 $205$\,ms; A40: mean $197$\,ms, p95 $338$\,ms
vs.\ a $K{=}4$ budget of $444$\,ms at 9\,FPS), so the deterministic
