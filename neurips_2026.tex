...
In both Speed Hex and Speed Go, the learned allocation shifts with clock pressure:
under the small clock setting ($T{=}300$) the policy relies much more heavily on the
smallest budget option, whereas under the larger clock setting ($T{=}4100$) compute
is spread much more evenly across the available options. Because the policy is
trained across many clock settings rather than specialized to a single budget, this
change reflects budget-conditioned generalization rather than retuning.
...
\begin{figure}[t]
  ...
  \caption{Planning depth frequency over normalized episode time (10 equal bins).
    ...
    option. For Speed Hex and Speed Go, the panels show early-clock ($T{=}300$)
    and late-clock ($T{=}4100$) evaluations from the same shared budget grid.}
  \label{fig:strategy}
\end{figure}
