...
%Y why is this hard
Crucially, there are state-dependent trade-offs between deliberating longer and reacting quickly.
Worse, deciding \textit{how much} to deliberate is itself a form of deliberation---naively, this risks infinite regress: a paralysis of planning about planning.
% Z our genius insight
We resolve this with a two-step process.
First, we train a standard MCTS policy.
Second, we train a \textit{lightweight} gating policy that decides how many search steps to invest at each state.
The key insight is that a single gate inference is orders of magnitude cheaper than even one MCTS rollout, so the meta-decision itself imposes negligible real-time cost and sidesteps the regress entirely.
Our gating policy consistently outperforms every fixed-budget baseline and hand-coded heuristic while using less average compute.
We validate the full pipeline in a true two-GPU real-time setup---and confirm that gate inference is never the bottleneck---across real-time Pac-Man, Tetris, Snake, Speed Hex, and Speed Go.
