...
\begin{abstract}
Problem: real-time RL.
Often times when human play games like atari, there are states where they need to be reactive, in other states, it requires the human to think and plan - planning here has a penalty because no-ops progress the environment (un like in traditional RL setups where the environment waits until an action has been submitted to progress to the next state). 

Contributions: We introduce the concept of implicity time pressure in real-tine RL environments, we show that alphazero nets have almost exact linearly proportional performance to inference time relationship, we take advantage of alphazero policies that can plan to increase performance but take proportionally longer inference time in these real-time RL environemnts, we show how to design these real-time RL environments either through no-op actions or a timed clock, we show that a simple light weight gating policy can be learned to learn state-conditioned compute allocation (deciding how much MCTS planning to spend) which both beats out using any individual mcts computation constantly, and uses it in an adaptive, resource-efficent strategy. We show these results in pacman, real-time tetris, timed sokoban, and two-player speed (clock) hex. We show that this setup can easily extended on real world experiments where an environment runs a seperate GPU in real-time.
