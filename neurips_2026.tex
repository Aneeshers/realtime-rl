...
In standard reinforcement learning (RL), the environment waits indefinitely
for the agent's decision, so computation is costless. We study real-time RL environments where this assumption fails, where the environment progresses
while the agent plans, and the cost of deliberation is paid in outcomes rather than
wall-clock seconds.
In this regime, planning quality improves with computation, but deeper planning also
takes longer to run, and that longer inference is exactly what advances the environment
during the delay.
...
We validate the full pipeline in a true two-GPU real-time setup, demonstrating that
the training protocol transfers directly to deployment without architectural changes.
We demonstrate these results across real-time Pac-Man, Tetris, timed Sokoban,
...
grows dense.
This adaptive allocation of cognitive effort is so natural that it goes
...
Whether a piece is falling, an opponent is moving, or a clock is ticking, the world
keeps progressing while the agent plans.
The cost of deliberation is not wall-clock latency but lost ground---steps taken by
the environment while the agent is still deciding.
A perfect action computed one moment too late may be far worse than a timely
good-enough action.
...
planning reliably improves decisions, but at a cost that grows with the depth of that
planning.
...
real-time settings that time is paid directly in game outcomes.
This dual property---planning quality scales up with compute, and real-time cost
scales up with inference time---makes adaptive compute allocation the natural lever
for performance.
...
\begin{enumerate}
  ...
  \item \textbf{Real-time deployment.}
    We validate the approach in a true two-GPU real-time setup---environment and
    committed-action policy on one GPU, MCTS on a second---showing that the
    committed-action training protocol transfers directly to deployment.
\end{enumerate}
...
suffice for competitive play, and that performance scales predictably with compute.
We exploit this scaling relationship as the foundation for adaptive allocation.
...
richer structure because the penalty is state-dependent and game-theoretic.
...
for the planner: the agent takes $K{-}1$ \emph{committed actions}---fast, approximate
...
actually experience.
The cost of deeper planning is exactly $K{-}1$ additional steps of world progression
before the carefully chosen action lands.
...
is exactly what advances the environment---or depletes the clock---before the
agent acts.
...
\begin{figure}[h]
  ...
  \todo{Figure: planning quality (return / win rate) and inference latency both plotted
    ...
    or as a two-panel figure. The key message: they scale together, linearly.}
...
\emph{how} to plan---the planner handles that---only \emph{when} the additional
quality gain is worth the additional real-time cost.
...
\begin{table}[h]
  \centering
  \caption{Per-decision latency and effective FPS (H100, batch size 1, 50 reps).}
  \label{tab:timing}
  \small
  \begin{tabular}{lcccc}
    \toprule
...
In Pac-Man, $K{=}4$ is the slowest option at ${\approx}9.9$ FPS; a target game speed
of 9 FPS ensures the planner finishes exactly within 4 frames.
In real-time Tetris, effective FPS is nearly constant across all $K$ (${\approx}42$--$53$):
simulation count and environment step count both scale linearly with $K$, so they cancel
in the ratio and the gating policy obtains deeper planning without sacrificing throughput.
...
${\approx}384$ms) completes within exactly 4 frames; shorter-depth options finish
with time to spare.
...
principle: planning that is cost-aware by construction.
...
invest computation proportionally to situational complexity---more for dense boards
and high-stakes decisions, less for easy ones.
...
and the strategy it learns transfers directly to two-GPU real-time deployment.
...
argmax of the base model's raw policy logits (no MCTS)---a single inexpensive
forward pass.
