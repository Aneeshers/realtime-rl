...
\begin{figure}[h]
  ...
  \caption{Speed Hex expected score vs.\ clock budget from the main WandB evaluation
    runs (primarily \texttt{gru\_ent100-500}, with the 8-simulation baseline drawn
    from \texttt{gru\_ent095-500}).
    Each curve is the gating policy's head-to-head expected score against one fixed
    or heuristic opponent, averaged across evaluation seeds.
    The dashed line marks parity ($0.5$ expected score). The five budgets used for
    the main-text summary are sampled from this broader sweep.}
