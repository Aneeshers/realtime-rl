...
Laslty, choosing how long to deliberate is fundamentally a problem of
...
\begin{figure}
    ...
    \caption{Illustration of an agent's actions}
    \label{fig:diagram}
\end{figure}
...
\end{equation}
where $\delta$ is a Dirac delta, which passes the newly emitted action
$\hat{a}_t$ forward to land at frame $t{+}1$.
In RTMDP, delay is a property of the system rather than a choice made
by the agent.

We generalize RTMDP by allowing the action delay to vary per decision.
At each meta-step the gating policy $\pigate$ chooses $K$, the number
of timesteps to plan over, and the selected action lands $K$ frames
later.
Because the environment evolves during those $K$ frames, we must also
specify what happens during the $K{-}1$ intermediate frames; we call
these \emph{committed actions}.

Each meta-decision can be viewed as selecting an
\emph{option}~\citep{sutton1999between,bacon2017optioncritic} of length
$K$: $K{-}1$ committed steps drawn from a fixed reflex distribution,
followed by one action selected by MCTS.
The induced process is a discrete-time semi-Markov decision process
(SMDP) with holding time $K$, meta-reward
\begin{equation}
  R_t \;=\; \sum_{k=0}^{K - 1} \gamma^k \, r_{t+k},
\end{equation}
and per-meta-step discount $\gamma^{K}$, as in the standard options
formulation.
The meta-Bellman equation is
\begin{equation}
  V(s_t)
  \;=\;
  \mathbb{E}_{K \sim \pigate}\!\left[
    R_t + \gamma^{K}\, V(s_{t + K})
  \right].
  \label{eq:smdp}
\end{equation}
In simulation this is a discrete SMDP with deterministic holding time
set by $K$; in deployment, MCTS runtime is slightly stochastic, so the
hardware system is closer to a continuous-time SMDP.
...
for the planner.
The agent emits $K{-}1$ committed actions. These are actions
drawn from the planner's own policy network (with no MCTS planning), computed via a single
forward pass in roughly two milliseconds, while the full MCTS runs
in the background.
...
We calibrate 32 MCTS simulations to one environment frame, which
corresponds to roughly 9 FPS on an H100
...
In clock environments, the clock is the only dynamic element of the
environment.
The agent still picks $K$ timesteps of deliberation, and the
environment still evolves over those $K$ timesteps, but here the
only thing that evolves is the player's clock, which decrements with
thinking time.
The board state itself does not change.

Committed actions are no-ops, and only the planner's chosen action
affects the board.
Cost-awareness is built into the value network rather than the search
tree: both players' remaining time is part of the observation during
self-play, so the value estimates already price in the cost of
thinking.
MCTS can therefore simulate only board-state dynamics while remaining
time-aware through the priors and values it consumes.

We instantiate this setting in Speed Hex ($11 \times 11$) and Speed
Go ($9 \times 9$), both built on \texttt{pgx}; in both cases the
central decision is not only \emph{which} move to play but \emph{how
much} of the remaining clock to spend before playing it.
We use a one-to-one mapping between MCTS simulations and clock ticks:
each simulation increments the player's clock by one unit.

\paragraph{Simulation-option calibration.}
For clock environments the equal-compute-per-frame constraint does not
apply, so we calibrate the simulation options separately to ensure
each option represents a meaningfully distinct tradeoff between
inference latency and planning quality.
Calibration details and the resulting option sets
$\{2, 8, 32, 128\}$ for Speed Hex and $\{16, 32, 64, 96\}$ for Speed
Go are in Appendix~\ref{app:sim_calibration}.
