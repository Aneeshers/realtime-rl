...
We verify empirically that this relationship is approximately linear
over the simulation ranges we study (Figure~\ref{fig:scaling}): each additional
batch of simulations yields a roughly equal gain in decision quality
and a roughly equal increase in real-time cost.
The full five-environment scaling results appear in Appendix~\ref{app:sim_calibration}.
...
\begin{figure}[h]
  ...
  \caption{More MCTS improves decisions, but it also costs more real
    time.
    Across Pac-Man, real-time Tetris, and 2-player Speed Hex, planning
    quality rises with simulation count while inference latency rises
    alongside it, creating the tradeoff that the gating policy must
    manage.
...
\end{figure}
\endgroup
This joint scaling makes the allocation problem tractable.
...
self-play before we freeze them and train a recurrent gating policy
on top; Appendix~\ref{app:implementation} gives the concrete
training details.
...
\begin{itemize}
    \item In Pac-Man, the best fixed policy ($K{=}3$, 96 sims/step) scores 2149; the gating policy reaches 2370 ($+10.3\%$) using only ${\approx}49$ simulations per frame on average.
    \item In real-time Tetris, the best fixed policy ($K{=}3$) scores 27.6; the gating policy reaches 45.6 ($+65\%$) using a bimodal strategy analyzed in Section~\ref{subsec:whattriggersdeeperplanning}.
    \item In Snake, the best fixed policy ($K{=}1$) scores 14.91; the gating policy reaches 16.54 ($+10.9\%$) by predominantly reacting quickly ($K{=}1$, 81.6\% of steps) while selectively invoking deeper planning as the board becomes constrained.
    \item In Speed Hex, averaged over the shared sampled clock budgets, the gating policy reaches an expected score of 0.58, compared with 0.43 for the best fixed-budget baseline ($K{=}128$) and 0.46 for the best heuristic baseline (greedy).
    \item In Speed Go, averaged over the same budgets, the gating policy reaches an expected score of 0.59, compared with 0.52 for the best fixed-budget baseline ($K{=}64$) and 0.50 for the best heuristic baseline (midpeak).
\end{itemize}

Across environments, the learned gate does not collapse to a single option; instead it learns dynamic, state- and budget-dependent allocation strategies that outperform both fixed-budget policies and hand-designed heuristics (Figure~\ref{fig:strategy}, Appendix Figure~\ref{fig:strategy_appendix}).
...
budget is most likely to change the decision. We visualize these situations in Figure~\ref{fig:interpretability}.

\paragraph{Pac-Man.}

Ghost proximity correlates with lower chosen budgets in Pac-Man: the policy prefers $K{=}1$ when the
...
across the four calibrated budgets, allocating deeper search substantially more often. Because
...
retuning. A strict-timeout Speed Hex control on a separate, wider budget sweep in Appendix~\ref{app:hex_timeout_control}
...
while real-time Tetris shifts toward deeper planning as boards become more constrained.
...
In real-time Tetris on H100, the deployed policy preserves the same strongly bimodal
