...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/deployment.pdf}
  \caption{Policies trained in simulation transfer cleanly to real asynchronous deployment.
    In the top-left timeline, GPU 0 keeps the environment moving with reflex
    actions while GPU 1 runs MCTS in parallel, so a larger chosen depth buys the
    planner more wall-clock time without pausing the game. The remaining panels
    summarize measured latency, average deadline misses, and return relative to
    simulation across H100, A100, and A40. Full per-environment, per-FPS, per-GPU
    results appear in Appendix~\ref{app:deployment_details}.}
