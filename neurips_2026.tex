...
\begin{tikzpicture}[
  ...
  state/.style={circle, draw, fill=gray!10, minimum size=6mm, inner sep=0pt},
  committed/.style={->, thick, draw=gray!70},
  planned/.style={->, thick, draw=black},
  bracket/.style={decorate, decoration={brace, amplitude=4pt, raise=2pt}, thick}
]
...
\end{tikzpicture}
\caption{A budgeted option $o_K$ over $K$ primitive frames. The agent emits
$K{-}1$ committed actions from $\pi_{\mathrm{reflex}}$ while MCTS runs
concurrently, and on the terminal frame the planner's chosen action is
applied. The gating policy returns to the meta level at $s_{t+K}$. In
clock environments, the committed steps are no-ops that only consume
clock.}
