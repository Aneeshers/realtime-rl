...
\begin{figure}[t]
  ...
  \caption{The committed-action planning mechanism.
    \emph{Left}: the gating policy reads the current game state (dense Tetris board shown
    as a heatmap) and outputs a depth distribution $P(K)$; for a dense, high-stakes board
    ...
    Figure generated by \texttt{plot\_method.py}.}
  \label{fig:method}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Interpretability: state features conditioned on chosen depth $K$
    ...
    piece type (all real data from RT\_KT rollouts).
    Figures generated by \texttt{plot\_pacman\_interpretability.py} and
    \texttt{plot\_tetris\_interpretability.py}.}
...
\begin{figure}[t]
  ...
  \caption{Planning depth frequency over normalized episode time (10 equal bins).
    ...
    Figure generated by \texttt{plot\_strategy.py}.}
  \label{fig:strategy}
\end{figure}
