...
\draw[committed] (s0) -- node[above, font=\small] {$\pi_{\mathrm{reflex}}$} (s1);
\draw[committed] (s1) -- node[above, font=\small] {$\pi_{\mathrm{reflex}}$} (s2);
\draw[committed] (s2) -- node[above, font=\small] {$\pi_{\mathrm{reflex}}$} (s3);
...
\draw[thick, draw=blue!60, line width=2pt]
  (m0) -- (m3)
  node[midway, below, font=\small, blue!60] {MCTS runs ($K$ frames)};

\draw[bracket] ($(s0.north) + (0,0.2)$) -- ($(s4.north) + (0,0.2)$)
  node[midway, above=8pt, font=\small] {option $o_K$ (holding time $K$)};

\node[font=\small, align=center] at (-1.4, 0) {gate\\ picks $K$};
\draw[->, thick] (-0.7, 0) -- (s0);
