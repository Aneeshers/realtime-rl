...
\section{AlphaZero and MCTS: A Self-Contained Primer}
\label{app:az_primer}
...
\textbf{Q: What does ``running a simulation'' mean in MCTS?}
...
\subsection*{How is the final action selected?}

\textbf{Q: How does the agent translate MCTS visit counts into an action?}
...
This is the expert-iteration loop --- search produces a better-than-raw-policy
distribution, and the network learns to imitate it, improving its prior for
future searches.

\subsection*{AlphaZero vs.\ MuZero}

\textbf{Q: What is the difference between AlphaZero and MuZero?}
...
\subsection*{Single-agent AlphaZero: expert iteration}

\textbf{Q: AlphaZero was designed for two-player zero-sum games. How does it transfer to single-agent settings?}
...
\subsection*{Scaling with JAX: vectorised parallel MCTS}

\textbf{Q: MCTS sounds inherently sequential. How do you run it efficiently at the scale needed for RL training?}
