...
The gating policy $\pigate(K \mid s_t)$ is the meta-policy of the SMDP
formalized in Section~\ref{sec:envs}: at each meta-step it samples
$K$, and the agent then executes $K{-}1$ committed actions followed
by the planner's chosen action.
The same formulation covers both environment types: in
committed-action environments the committed actions advance the world
and contribute rewards, while in clock environments they are no-ops
that contribute zero reward, so the meta-reward $R_t$ reduces to the
reward of the planner's chosen action while the $\gamma^K$ factor
still discounts the elapsed clock cost.
...
\begin{itemize}
    ...
    \item In real-time Tetris, the best fixed policy ($K{=}3$) scores 27.6; the gating policy reaches 45.6 ($+65\%$) using a bimodal strategy analyse in Section~\ref{subsec:whattriggersdeeperplanning}.
...
Against the fixed-budget baselines and the greedy, midpeak, and random
heuristics, the gating policy remains above parity on average and
outperforms these opponents overall.
...
exactly one timestep via a 1-step delay RTMDP state augmentation; however, it is insufficient for variable-budget MCTS spanning
