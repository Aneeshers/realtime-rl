...
Instead we here study environments that progress while waiting for the agent's action, which naturally creates a \textit{cost of deliberation}.
%Y why is this hard
Crucially, in these settings, there are state-dependent trade-offs between taking more time to deliberate or being able to react quickly. Furthermore, deliberating how much to deliberate, in the limit, risks complete paralysis.  
% Z our genius insight
To address this, we propose a simple two step process: 
In the first step we train a standard MCTS policy and in the second step we train a \textit{lightweight} meta-policy that decides how many search steps to invest at a given step.
Our meta-policy consistently outperforms every fixed-budget baseline and handcoded heuristics while using less average compute.
Moreover, we validate the full pipeline in a true two-GPU real-time setup. We demonstrate these results across real-time versions of Pac-Man, Tetris, Snake, and two clocked games: Speed Hex and Speed Go.
...
~\cite{russell1991right,korf1990rta,dean1988analysis,zilberstein1996using,likhachev2003ara}.
...
~\cite{lieder2017strategy,griffiths2019doing,callaway2018learning}. Our work
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    Snake             & Committed-action & 32, 64, 96, 128 \\[4pt]
    \multicolumn{3}{l}{\textit{Clock}} \\[2pt]
    Speed Hex (11$\times$11) & Explicit clock & 2, 8, 32, 128 \\
    Speed Go (9$\times$9) & Explicit clock & 16, 32, 64, 96 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{Learning when to spend compute beats any fixed compute budget.
    ...
    shared sampled clock budgets $T \in \{300, 1200, 2300, 3500, 4800\}$.}
...
\begin{figure}[t]
  ...
  \caption{Clock pressure changes how compute is allocated. In both Speed Hex and
    ...
    available ($T{=}4100$). Lines show how often each budget option is selected
    over normalized episode time. Additional per-environment strategy profiles
    appear in Appendix Figure~\ref{fig:strategy_appendix}.}
  \label{fig:strategy}
\end{figure}
...
reliable on A100 except near the tightest deadline regimes.
For example, in Tetris at 9\,FPS, where the $K{=}4$ budget is 444\,ms, mean latency
on H100 is only 124\,ms.
The failures that do appear are concentrated near the deadline boundary:
Tetris on A40 breaks down at 12\,FPS, Pac-Man is the most timing-sensitive
environment overall, and Snake remains robust throughout.

Return transfer is similarly strong.
...
\begin{figure}[h]
  ...
  \caption{\textbf{Two-GPU real-time deployment pipeline.}
    At each meta-decision, the current game state is transferred to GPU~1, which
    immediately begins MCTS while GPU~0 keeps the environment running at full speed
    by replaying the committed (reflex) action sequence.
    A larger chosen depth $K$ simply gives the planner more wall-clock time---the
    game never pauses.
    Once planning finishes, the new committed-action sequence takes effect at the
    next meta-decision boundary.
    The left schematic shows the two-GPU loop; the right Gantt chart shows the
    resulting execution trace at 9\,FPS, with a $K{=}4$ meta-step spanning four
    111\,ms frames followed immediately by a $K{=}1$ recovery step.}
...
\begin{figure}[h]
  ...
  \caption{\textbf{Simulation-trained policies transfer cleanly to hardware deployment.}
    \emph{Left:} MCTS latency distributions across three environments and three GPU
    classes (H100, A100, A40), pooled over all measured frame rates (8--12\,FPS).
    \emph{Centre:} Average deadline miss rate per GPU class; H100 and A100 stay
    well below 1\% across all environments, while A40 breaks down only at the
    tightest deadline regimes.
    \emph{Right:} Episode return relative to the simulation baseline (100\,\%) at
    9\,FPS, showing that sim-trained policies retain nearly full performance on
    real hardware.
    Full per-environment, per-FPS, per-GPU results appear in
    Appendix~\ref{app:deployment_details}.}
