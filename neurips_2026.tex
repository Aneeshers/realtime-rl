...
We substantiate these contributions empirically.
...
We qualitatively find that the agent with the gating policy learns to plan for longer when it needs to and acts promptly when it is under pressure; we demonstrate examples across our environments of when this happens.
...
To validate the environments we designed to capture the challenges of real-time decision-making and how we trained with them, we transfer the learned gating policy to a real-time hardware setting where the environment runs on one GPU and the agent runs on a second GPU without any retraining or architectural changes to demonstrate that it performs effectively.
