...
\begin{figure}[h]
  ...
  \caption{Planning quality and real-time inference latency co-scale with simulation
    ...
    A100, and V100.
    Shaded band shows $\pm$SE for H100.
    Filled markers indicate all data points (both measured and linearly extrapolated).}
  \label{fig:scaling}
\end{figure}
...
\begin{table}[h]
  ...
  \begin{tabular}{llll}
    ...
    Real-Time Tetris  & Committed-action & 2000 steps & 32, 64, 96, 128 \\
    Snake             & Committed-action & Variable   & \{1, 2, 4\}  \emph{(training)} \\[4pt]
...
In Snake, the strategy shifts dynamically with episode length: $K{=}1$ dominates the early game when the snake is short, while reliance on $K{=}4$ grows steadily as the snake elongates and maneuvering becomes constrained.
...
\begin{figure}[t]
  ...
  \caption{Interpretability: state features conditioned on chosen depth $K$
    ...
    mean nearest-ghost distance by chosen $K$.
...
\begin{figure}[h]
  ...
  \caption{Validation of the real-time, asynchronous 2-GPU deployment.
    \emph{Left:} Timeline of execution showing the environment and fast committed-action
    policy ($\pi_{\approx}$) operating continuously on GPU 1 while the MCTS planner
    evaluates deeper meta-decisions asynchronously on GPU 2.
    \emph{Center-Left:} Latency distributions for the MCTS evaluation step across different
    hardware architectures (placeholder data).
    \emph{Right Panels:} Zero-shot deployment performance on H100 hardware for Tetris RT (episode return) and Speed Hex (win rate) matches simulated training performance
    (placeholder data), validating that the committed-action training protocol perfectly models the deployment delay.}
