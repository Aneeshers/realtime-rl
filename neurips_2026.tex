...
During ticks $t{+}1$ through $t{+}3$, a fast approximate policy ($\pi_\approx$)
...
on the final step, a placement the approximate policy would have missed.
...
\begin{figure}[t]
  ...
  \caption{Committed-action execution in real-time Tetris with $K{=}4$.
    ...
    Ticks $t{+}1$–$t{+}3$ (gray): committed actions from the approximate policy
    $\pi_\approx$ move the piece left while gravity falls it one row per tick.
    ...
    Figure generated by \texttt{plot\_tetris\_committed.py}.}
