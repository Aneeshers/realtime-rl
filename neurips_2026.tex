...
learn per-query thinking budgets, these are conceptual twins of our gating policy in
...
\begin{figure}[h]
  ...
  \caption{Planning quality and real-time inference cost co-scale with simulation count
    ...
    \emph{Red dashed}: per-step inference latency (ms).
    Both metrics increase monotonically with simulation count; shaded bands show $\pm$SE.
    Filled markers indicate measured data points; open markers indicate linear extrapolations
    to the shared grid $s \in \{2,4,8,16,32,64,128,256\}$.
    Speed Hex latency is synthetic-linear (no hardware timing available from tournament logs).}
  \label{fig:scaling}
\end{figure}
